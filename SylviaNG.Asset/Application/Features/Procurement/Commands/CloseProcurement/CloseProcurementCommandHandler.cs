...
public class CloseProcurementCommandHandler : IRequestHandler<CloseProcurementCommand>
{
    ...
    public CloseProcurementCommandHandler(
    ...
    public async Task Handle(CloseProcurementCommand request, CancellationToken cancellationToken)
    {
        ...
        _service.Close(requisition, userId, _currentUser.FullName ?? "Unknown", _currentUser.Role?.ToString(), request.Comment);
        ...
        await _auditLogger.LogAsync("ProcurementClosed", nameof(Requisition), requisition.Id, cancellationToken: cancellationToken);
    }
}
